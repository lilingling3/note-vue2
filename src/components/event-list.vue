    ...
    <div class="event-box" :style="{'height':'auto','display':'block'}">
        <ul>
          <li class="event-list" v-for="value in getTodo">
              <input type="checkbox" :key="value.id" @click="moveToDone(value.id)"></input>
              <div>{{value.content}}</div>
              <button class="cancel-btn" @click="moveToCancel(value.id)">取消</button>
          </li>
        </ul>
    </div>
    ...
    <div class="event-box">
        <ul>
          <li class="event-list" v-for="value in getDone">
              <input type="checkbox" :key="value.id" checked @click="moveToDo(value.id)"></input>
              <div>{{value.content}}</div>
  ...
  methods:{
    ...
    closeCollapse(event){
      let ulElement = event.currentTarget.nextElementSibling,
          ...
          console.log(ulElement.style.height)
          setTimeout(function() {
